...
# CONFIGURATION
# ══════════════════════════════════════════════════════════════════
APP_TITLE       = "Fasterfood Point of Sale System"
DB_FILE         = "fasterfood_pos.db"
VAT_RATE        = 0.12
MAX_ATTEMPTS    = 5
LOCKOUT_SECS    = 300
...
STORE_INFO = {
    "name":    "FASTERFOOD",
    "branch":  "SM North EDSA Branch",
    "address": "SM North EDSA Complex, Quezon City",
    "tel":     "(02) 8-FAST-EAT",
    "tin":     "987-654-321-000",
}

# ══════════════════════════════════════════════════════════════════
# COLOR PALETTE  — dark navy + electric orange + teal
# ══════════════════════════════════════════════════════════════════
PRIMARY   = "#FF6B35";  PRIMARY_D  = "#E55A24"
ACCENT    = "#00B4D8";  ACCENT_D   = "#0096B7"
WHITE     = "#FFFFFF";  BLACK      = "#1C1C1C"
GRAY      = "#6B7280";  LGRAY      = "#F1F5F9"
BG        = "#EEF2F7";  CARD       = "#FFFFFF"
SIDEBAR   = "#0D1B2A";  SIDEBAR2   = "#08111C"
SUCC      = "#059669";  SUCC_D     = "#047857"
DANGER    = "#DC2626";  BORDER     = "#DDE3EC"
...
MENU = {
    "🍔 Smash Burgers": [
        ("Classic Smash Solo",         119.00),
        ("Classic Smash Meal",         165.00),
        ("Double Smash Solo",          159.00),
        ("Double Smash Meal",          205.00),
        ("Triple Smash Solo",          199.00),
        ("Bacon Smash Solo",           149.00),
        ("Spicy Smash Solo",           145.00),
    ],
    "🌯 Wraps": [
        ("Grilled Chicken Wrap",       129.00),
        ("Crispy Chicken Wrap",        129.00),
        ("Beef & Cheese Wrap",         145.00),
        ("Veggie Wrap",                109.00),
        ("Buffalo Chicken Wrap",       139.00),
        ("Tuna Melt Wrap",             135.00),
    ],
    "🍟 Fries & Sides": [
        ("Classic Fries Regular",       55.00),
        ("Classic Fries Large",         75.00),
        ("Loaded Cheese Fries",         99.00),
        ("Garlic Parmesan Fries",       89.00),
        ("Onion Rings",                 79.00),
        ("Mozza Sticks (4 pcs)",        89.00),
        ("Coleslaw Cup",                45.00),
    ],
    "🌮 Tacos": [
        ("Beef Taco",                   79.00),
        ...
        ("Fish Taco",                   89.00),
        ("Shrimp Taco",                 99.00),
        ("Taco Box (3 pcs)",           219.00),
        ("Taco Box (5 pcs)",           349.00),
    ],
    "🥗 Bowls": [
        ("Teriyaki Chicken Bowl",      155.00),
        ("Beef Bulgogi Bowl",          175.00),
        ("Salmon Poke Bowl",           199.00),
        ("Vegan Power Bowl",           145.00),
        ("Grilled Veggie Bowl",        135.00),
        ("Katsu Chicken Bowl",         165.00),
    ],
    "🥤 Drinks": [
        ("Iced Tea Regular",            45.00),
        ("Iced Tea Large",              59.00),
        ("Lemonade",                    55.00),
        ("Strawberry Lemonade",         65.00),
        ("Milkshake – Vanilla",         89.00),
        ("Milkshake – Chocolate",       89.00),
        ("Milkshake – Strawberry",      89.00),
        ("Bottled Water",               25.00),
    ],
    "🍦 Frozen Treats": [
        ("Soft Serve – Vanilla",        55.00),
        ("Soft Serve – Chocolate",      55.00),
        ("Sundae – Caramel Drizzle",    69.00),
        ("Sundae – Mixed Berry",        69.00),
        ("Frozen Yogurt Cup",           79.00),
        ("Ice Cream Sandwich",          65.00),
    ],
}
...
class Database:
    ...
    def next_order_num(self) -> str:
        ...
        return f"FF-{today}-{(row[0] or 0) + 1:04d}"
...
class UserFormDialog(tk.Toplevel):
    ...
    mode = "edit" → username disabled, password optional
    """
    ...
    def _build(self):
        ...
        tk.Label(hdr, text=f"{icon}  {lbl}",
                 font=F_H2, bg=SIDEBAR, fg=PRIMARY).pack(padx=16)
        ...
        for val, lbl in [("cashier", "👷 Cashier"), ("admin", "🔑 Admin")]:
            tk.Radiobutton(role_f, text=lbl, variable=self._role_var, value=val,
                           font=F_BODY, bg=WHITE, selectcolor=ACCENT,
...
class LoginFrame(tk.Frame):
    ...
    def _build(self):
        ...
        strip = tk.Frame(self, bg=PRIMARY, width=8)
        ...
        # Brand mark
        brand_f = tk.Frame(card, bg=WHITE)
        brand_f.pack(pady=(0, 6))
        try:
            logo_img = Image.open("FasterfoodLogo.png")
            logo_img = logo_img.resize((96, 96))
            self.logo_photo = ImageTk.PhotoImage(logo_img)
            tk.Label(brand_f, image=self.logo_photo, bg=WHITE).pack()
        except Exception:
            icon_lbl = tk.Label(brand_f, text="⚡", font=("Helvetica", 60), bg=WHITE, fg=PRIMARY)
            icon_lbl.pack()

        tk.Label(card, text="FASTERFOOD", font=("Helvetica", 22, "bold"),
                 bg=WHITE, fg=SIDEBAR).pack()
        tk.Label(card, text="Faster. Better. Tastier.", font=("Helvetica", 10),
                 bg=WHITE, fg=GRAY).pack(pady=(2, 20))

        # Divider
        div = tk.Frame(card, bg=PRIMARY, height=3)
        ...
        tk.Label(form, text="USERNAME", font=("Helvetica", 9, "bold"),
                 bg=WHITE, fg=GRAY, anchor="w").pack(fill="x", pady=(0, 4))
        ...
        self._uname_e.pack(fill="x", ipady=9, pady=(0, 14))
        ...
        self._pwd_e.pack(side="left", fill="x", expand=True, ipady=9)
        self._eye_on = False
        ...
        self._login_btn = tk.Button(
            ...
            bg=PRIMARY, fg=WHITE, activebackground=PRIMARY_D, activeforeground=WHITE,
            relief="flat", cursor="hand2",
            command=self._do_login, pady=12, width=28)
        self._login_btn.pack(pady=(16, 0))

        # Footer
        tk.Label(card, text=f"{STORE_INFO['branch']}  •  {STORE_INFO['address']}",
                 font=("Helvetica", 8), bg=WHITE, fg=GRAY).pack(pady=(18, 0))
...
class POSFrame(tk.Frame):
    ...
    def _build(self):
        ...
        sidebar = tk.Frame(content, bg=SIDEBAR, width=185)
        ...
        right = tk.Frame(content, bg=CARD, width=355)
    ...
    def _build_header(self):
        ...
        accent_bar = tk.Frame(hdr, bg=PRIMARY, width=5)
        accent_bar.pack(side="left", fill="y")

        try:
            logo_img = Image.open("FasterfoodMascot.png")
            logo_img = logo_img.resize((44, 44))
            self.logo_photo = ImageTk.PhotoImage(logo_img)
            tk.Label(hdr, image=self.logo_photo, bg=SIDEBAR).pack(side="left", padx=(10, 4), pady=8)
        except Exception:
            tk.Label(hdr, text="⚡", font=("Helvetica", 24), bg=SIDEBAR, fg=PRIMARY
                     ).pack(side="left", padx=(10, 4), pady=8)

        left = tk.Frame(hdr, bg=SIDEBAR)
        left.pack(side="left", padx=4, pady=8)
        tk.Label(left, text="FASTERFOOD", font=("Helvetica", 14, "bold"),
                 bg=SIDEBAR, fg=WHITE).pack(side="left")
        tk.Label(left, text=f"  ·  {STORE_INFO['branch']}",
                 font=("Helvetica", 10), bg=SIDEBAR, fg="#5A7FA0").pack(side="left")

        right = tk.Frame(hdr, bg=SIDEBAR)
        right.pack(side="right", padx=14, pady=8)

        if self.auth.is_admin:
            tk.Button(right, text="⚙ Admin", font=F_SM, bg=NAVY_MID, fg=ACCENT,
                      relief="flat", cursor="hand2", command=self._show_admin
                      ).pack(side="right", padx=(6, 0))

        tk.Button(right, text="⏻ Logout", font=F_SM, bg=DANGER, fg=WHITE,
                  activebackground="#991A1A", relief="flat", cursor="hand2",
                  command=self._logout).pack(side="right", padx=(6, 0))

        self._today_lbl = tk.Label(right, text="", font=F_SM, bg=SIDEBAR, fg="#5A7FA0")
        self._today_lbl.pack(side="right", padx=(6, 0))

        self._clock_lbl = tk.Label(right, text="", font=("Courier New", 10),
                                    bg=SIDEBAR, fg=WHITE)
        self._clock_lbl.pack(side="right", padx=(6, 0))

        self._session_lbl = tk.Label(right, text="", font=("Helvetica", 8),
                                      bg=SIDEBAR, fg="#5A7FA0")
        self._session_lbl.pack(side="right", padx=(6, 0))

        tk.Label(right, text=f"👤  {self.auth.user['username'].title()}",
                 font=("Helvetica", 11, "bold"), bg=SIDEBAR, fg=PRIMARY
                 ).pack(side="right", padx=(6, 0))

        self._refresh_today_counter()

    def _build_sidebar(self, parent):
        # Brand tagline in sidebar header
        hdr = tk.Frame(parent, bg=SIDEBAR2, pady=10)
        hdr.pack(fill="x")
        tk.Label(hdr, text="MENU", font=("Helvetica", 9, "bold"),
                 bg=SIDEBAR2, fg=PRIMARY, pady=0).pack()
        tk.Label(hdr, text="CATEGORIES", font=("Helvetica", 7),
                 bg=SIDEBAR2, fg="#3A5A78", pady=0).pack()

        for cat in CATEGORIES:
            btn = tk.Button(
                parent, text=cat,
                font=("Helvetica", 10, "bold"), anchor="w", padx=14, pady=12,
                bg=SIDEBAR, fg="#B0C4D8",
            ...
            def _enter(e, b=btn, c=cat):
                if c != self._cur_cat: b.configure(bg=NAVY_MID, fg=WHITE)
            def _leave(e, b=btn, c=cat):
                if c != self._cur_cat: b.configure(bg=SIDEBAR, fg="#B0C4D8")
            btn.bind("<Enter>", _enter)
            btn.bind("<Leave>", _leave)
            self._cat_btns[cat] = btn

        foot = tk.Frame(parent, bg=SIDEBAR2)
        foot.pack(side="bottom", fill="x", pady=10)
        tk.Label(foot, text=f"VAT: {int(VAT_RATE*100)}% inclusive",
                 font=F_SM, bg=SIDEBAR2, fg="#3A5A78").pack()
        tk.Label(foot, text="v1.0  © Fasterfood POS",
                 font=("Helvetica", 8), bg=SIDEBAR2, fg="#2A3A4A").pack()
    ...
    def _build_menu_area(self, parent):
        ...
        self._cat_title = tk.Label(title_bar, text="", font=F_H1, bg=BG, fg=SIDEBAR,
                                    padx=16, pady=12, anchor="w")
        self._cat_title.pack(side="left", fill="x")

        # Orange underline
        underline = tk.Frame(parent, bg=PRIMARY, height=3)
        underline.pack(fill="x", side="top")
    ...
    def _build_right_panel(self, parent):
        # Order header with teal accent
        rh = tk.Frame(parent, bg=NAVY_MID, pady=8)
        rh.pack(fill="x", side="top")
        tk.Label(rh, text="ORDER SUMMARY", font=("Helvetica", 11, "bold"),
                 bg=NAVY_MID, fg=WHITE).pack(side="left", padx=12)
        self._badge = tk.Label(rh, text="0 items", font=F_SM,
                                bg=PRIMARY, fg=WHITE, padx=8, pady=2)
    ...
    def _build_cart_tree(self, parent):
        ...
        self._cart_tree.column("name",  width=150, stretch=True,  anchor="w")
        self._cart_tree.column("qty",   width=38,  stretch=False, anchor="center")
        self._cart_tree.column("price", width=72,  stretch=False, anchor="e")
    ...
    def _build_controls(self, parent):
        ...
        tk.Button(act_f, text="✖  Clear All", font=F_SM, bg="#3A5A78", fg=WHITE,
                  activebackground=SIDEBAR, relief="flat", cursor="hand2",
        ...
        for opt in ("Dine In", "Take Out"):
            tk.Radiobutton(dine_f, text=opt, variable=self._dine_var, value=opt,
                           ...
                           selectcolor=PRIMARY, activebackground=CARD
        ...
        tk.Label(total_row, text="TOTAL DUE", font=("Helvetica", 11, "bold"),
                 bg=CARD, fg=SIDEBAR).pack(side="left")
        self._total_lbl = tk.Label(total_row, text="₱0.00",
                                    font=("Helvetica", 18, "bold"), bg=CARD, fg=PRIMARY)
        ...
        pay_f.pack(fill="x")
        tk.Label(pay_f, text="Payment Method:", font=F_H3, bg=CARD, fg=BLACK
        ...
        for lbl, val in [("💵 Cash","Cash"),("💳 Card","Card"),("📱 E-Wallet","E-Wallet")]:
            b = tk.Button(btn_row, text=lbl, font=("Helvetica", 9, "bold"),
                          bg=LGRAY, fg=BLACK, activebackground=ACCENT_D, activeforeground=WHITE,
                          relief="flat", cursor="hand2",
    ...
    def _load_category(self, category: str):
        for cat, btn in self._cat_btns.items():
            if cat == category:
                ...
                btn.configure(bg=SIDEBAR, fg="#B0C4D8")
        ...
        for i, (name, price) in enumerate(MENU[category]):
            ...
            # Top colored stripe
            stripe = tk.Frame(card, bg=PRIMARY, height=4)
            stripe.pack(fill="x")

            tk.Label(card, text=name, font=F_H3, bg=CARD, fg=SIDEBAR,
                     wraplength=158, justify="center", pady=10).pack(fill="x", padx=8)
            tk.Label(card, text=f"₱{price:,.2f}",
                     font=("Helvetica", 13, "bold"), bg=CARD, fg=PRIMARY).pack()

            def _add_item(n=name, p=price, frm=card, st=stripe):
                self._add_to_cart(n, p)
                st.configure(bg=SUCC)
                frm.configure(highlightbackground=SUCC)
                frm.after(260, lambda: (
                    st.configure(bg=PRIMARY),
                    frm.configure(highlightbackground=BORDER)
                ))
            tk.Button(card, text="＋ ADD TO ORDER",
                      font=("Helvetica", 9, "bold"),
                      bg=SIDEBAR, fg=WHITE, activebackground=NAVY_MID,
    ...
    def _set_pay_method(self, method: str):
        ...
        for val, btn in self._pay_btns.items():
            if val == method:
                ...
                btn.configure(bg=LGRAY, fg=BLACK)
    ...
    def _check_session_timeout(self):
        ...
        if remaining < 300:
            self._session_lbl.configure(text=f"⏱ Session: {m}m {s}s", fg=PRIMARY)
        else:
            self._session_lbl.configure(text=f"⏱ {m}m", fg="#5A7FA0")
...
class PaymentDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(hdr, text="⚡  PROCESS PAYMENT", font=F_H1, bg=SIDEBAR, fg=WHITE).pack()
        badge_col = {"Cash": SUCC, "Card": INFO, "E-Wallet": "#7C3AED"}.get(self.method, GRAY)
        ...
        due = tk.Frame(self, bg=LGRAY, pady=14); due.pack(fill="x")
        tk.Label(due, text="AMOUNT DUE", font=("Helvetica", 9, "bold"),
                 bg=LGRAY, fg=GRAY).pack()
        tk.Label(due, text=f"₱{self.total:,.2f}",
                 font=("Helvetica", 30, "bold"), bg=LGRAY, fg=PRIMARY).pack()
    ...
    def _build_cash(self):
        ...
        self._display = tk.Label(entry_f, text="₱0",
        ...
        for r, row in enumerate(keys):
            for c, key in enumerate(row):
                ...
                tk.Button(np_f, text=key, font=("Helvetica", 14, "bold"),
                          ...
                          fg=DANGER if special else SIDEBAR,
    ...
    def _build_digital(self):
        ...
        tk.Button(self, text="✓   PAYMENT RECEIVED",
                  font=("Helvetica", 12, "bold"),
...
class ReceiptWindow(tk.Toplevel):
    ...
    def _build(self):
        bar = tk.Frame(self, bg=SIDEBAR, pady=7); bar.pack(fill="x")
        tk.Button(bar, text="🖨  Print / Save", font=F_BODY, bg=NAVY_MID, fg=WHITE,
                  relief="flat", cursor="hand2", command=self._print
                  ).pack(side="left", padx=10)
        tk.Button(bar, text="New Order  →", font=F_BODY, bg=PRIMARY, fg=WHITE,
                  activebackground=PRIMARY_D, relief="flat",
    ...
    def _render(self):
        ...
        two_col("TOTAL DUE", f"₱{o['total']:>8,.2f}"); div("═"); ln()
        ...
        ln(); div(); ln("Thank you for choosing FASTERFOOD!", "c")
        ln('"Faster. Better. Tastier."', "c"); div(); ln("*** Customer Copy ***", "c"); ln()
...
class AdminPanel(tk.Toplevel):
    ...
    def _build(self):
        hdr = tk.Frame(self, bg=SIDEBAR, pady=10); hdr.pack(fill="x")
        # Orange left accent
        tk.Frame(hdr, bg=PRIMARY, width=5).pack(side="left", fill="y")
        tk.Label(hdr, text="⚙   ADMIN PANEL", font=F_H1, bg=SIDEBAR, fg=WHITE
                 ).pack(side="left", padx=20)
        tk.Label(hdr, text=f"Logged in as: {self.auth.user['username']}",
                 font=F_SM, bg=SIDEBAR, fg="#5A7FA0").pack(side="left", padx=10)
    ...
    def _view_order_detail(self, event):
        ...
        two("TOTAL DUE", f"₱{order['total']:>8,.2f}"); div("═"); ln()
    ...
    def _refresh_summary(self):
        ...
        tk.Label(self._sum_content, text="Today's Sales Summary",
                 font=F_H1, bg=BG, fg=SIDEBAR).pack(pady=(0, 20))
        # Accent bar
        tk.Frame(self._sum_content, bg=PRIMARY, height=3).pack(fill="x", pady=(0, 16))
        for label, value, color in [
            ("Total Orders",    str(cnt),           SIDEBAR),
            ("Total Revenue",   f"₱{rev:,.2f}",     PRIMARY),
            ("Average Order",   f"₱{avg:,.2f}",     SUCC),
        ]:
            card = tk.Frame(self._sum_content, bg=CARD, padx=20, pady=16, relief="solid", bd=1)
            card.pack(fill="x", pady=6)
            # Left stripe per card
            tk.Frame(card, bg=color, width=4).pack(side="left", fill="y", padx=(0, 14))
            info = tk.Frame(card, bg=CARD)
            info.pack(side="left")
            tk.Label(info, text=label, font=F_BODY, bg=CARD, fg=GRAY, anchor="w").pack(anchor="w")
            tk.Label(info, text=value, font=("Helvetica", 26, "bold"),
                     bg=CARD, fg=color, anchor="w").pack(anchor="w")
        tk.Button(self._sum_content, text="🔄  Refresh", font=F_SM,
                  bg=LGRAY, relief="flat", cursor="hand2",
    ...
    def _build_users_tab(self):
        ...
        tk.Button(bar, text="🔓  Unlock Selected",
                  font=("Helvetica", 9, "bold"),
                  bg=WARN, fg=WHITE, relief="flat", cursor="hand2",
                  command=self._unlock_selected_user
                  ).pack(side="left", padx=(6, 0), ipadx=8, ipady=3)
        tk.Button(bar, text="🔄  Refresh",
                  font=F_SM, bg=LGRAY, fg=BLACK, relief="flat", cursor="hand2",
    ...
    def _reload_users(self):
        ...
        for row in self.db.get_all_users():
            ...
            self._usr_tree.insert("", "end", iid=str(row["id"]), tags=(tag,), values=(
                ...
                row["created_at"] or "—", row["last_login"] or "Never",
                row["failed"], status,
            ))
        self._usr_tree.tag_configure("locked",    background="#FFEBEE")
        self._usr_tree.tag_configure("admin_row", background="#FFF3E0")
    ...
    def _add_user(self):
        UserFormDialog(self, self.db, self.auth, mode="add", on_save=self._reload_users)

    def _edit_user(self):
        row = self._get_selected_user_row()
        if row is None:
            messagebox.showinfo("No Selection", "Please select a user to edit.", parent=self)
            return
    ...
    def _delete_user(self):
        ...
        self._reload_users()
        messagebox.showinfo("Deleted", f"User '{uname}' has been deleted.", parent=self)

    def _unlock_selected_user(self):
        row = self._get_selected_user_row()
        if row is None:
            messagebox.showinfo("No Selection", "Please select a user to unlock.", parent=self)
            return
        ...
        if messagebox.askyesno("Unlock User",
            ...
            self._reload_users()
            messagebox.showinfo("Success", f"Account '{name}' has been unlocked.", parent=self)
    ...
    def _build_login_log_tab(self):
        ...
        for txt, val in [("All","All"),("✅ Success","Success"),("✖ Failed","Failed")]:
            tk.Radiobutton(bar, text=txt, variable=self._log_filter, value=val,
                           font=F_SM, bg=BG, fg=BLACK, selectcolor=PRIMARY,
        ...
        self._log_tree.tag_configure("success", background="#D1FAE5")
        self._log_tree.tag_configure("fail",    background="#FEE2E2")
    ...
    def _build_session_tab(self):
        ...
        self._ses_tree.tag_configure("active",  background="#DBEAFE")
        self._ses_tree.tag_configure("timeout", background="#FEF3C7")
        self._reload_sessions()
        tk.Label(self._ses_tab,
                 text="Blue = active session  |  Amber = session ended by timeout",
                 font=F_SM, bg=BG, fg=GRAY).pack(pady=4)
    ...
    def _build_audit_tab(self):
        ...
        for txt in ["All","LOGIN","LOGOUT","ORDER_PLACED",
